...
bot.callbackQuery('menu:subscription', async (ctx) => {
  ...
  try {
    ...
    const squadNames = (rmUser.activeInternalSquads ?? []).map((s: any) => s.name).join(', ')
...
bot.callbackQuery('menu:balance', async (ctx) => {
  ...
  const history = await prisma.balanceTransaction.findMany({ where: { userId: user.id }, orderBy: { createdAt: 'desc' }, take: 3 })
  const last3 = history
  ...
  const text = `💰 *Баланс:* ${balance} ₽${txLines}`
